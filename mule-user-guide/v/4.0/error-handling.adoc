...
//REVIEW: Update with error-handler scope example.
...
When a system exception strategy occurs, Mule sends an exception notification to registered listeners, logs the exception, and — if the exception was caused by a connection failure — executes the link:/mule-user-guide/v/4.0/configuring-reconnection-strategies[reconnection strategy]. System Exception Strategies are not configurable in Mule.
...
Consult the individual link:/mule-user-guide/v/4.0/catch-exception-strategy[catch], link:/mule-user-guide/v/4.0/rollback-exception-strategy[rollback] or link:/mule-user-guide/v/4.0/choice-exception-strategy[choice] exception strategy pages for details on how to create them globally; consult the link:/mule-user-guide/v/4.0/reference-exception-strategy[reference] exception strategy page for details on how to reuse them globally.
...
*Until Successful* behaves similarly to a rollback exception strategy. This scope attempts to route a message through its child flow until the message is processed successfully. However, you can define the maximum number of processing attempts the Until Successful scope undertakes before it reverts to handling the message as though it were an exception. You can configure a *Failure Expression*, an *Ack Expression*, or a *Dead Letter Queue Reference* to instruct the scope on how to manage messages that it cannot process. In this respect, Until Successful scope behavior is similar to a link:/mule-user-guide/v/4.0/rollback-exception-strategy[rollback exception strategy]. Refer to the link:/mule-user-guide/v/4.0/scopes[Scopes] documentation for details.
...
* To understand how to configure Mule’s exception strategies to handle the most common error handling use cases, read link:/mule-user-guide/v/4.0/exception-strategy-most-common-use-cases[Exception Strategy Most Common Use Cases].
